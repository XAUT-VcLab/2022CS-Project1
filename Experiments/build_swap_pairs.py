...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Build swap pairs for DecTalk3D condition swapping experiments.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Maximum number of directed pairs sampled from each group"
    )
...
def parse_video_token(video_token: str) -> Dict:
    """
    严格按 DecTalk3D 的 DataProcess/Dataload.py 对齐：
    parts[0] -> person_id
    parts[2] -> emotion
    parts[3] -> intensity
    其余部分作为 sentence_key
...
def build_sample_records(dataset: CustomDataset) -> List[Dict]:
    """
    不加载 h5 内容，只基于 dataset.files + extract_video_token 构建元信息。
    这是 DecTalk3D dataloader 已有接口。
    """
    records = []
    ...
    for _, items in grouped.items():
        ...
        for a, b in candidates:
            pair_list.append({
                ...
                "target_intensity": b["intensity"],  # 应相同
            })
